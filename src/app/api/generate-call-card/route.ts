...
export async function POST(request: Request) {
  try { const { email } = z.object({ email: z.string().email() }).parse(await request.json()); const context = await getContactContext(email); if (!context) return NextResponse.json({ error: "No contact found for this email." }, { status: 404 }); const baseCard = fallback(context); let card = baseCard; let model = "rule-based"; try { const result = await openRouterJson<typeof card>(callCardPrompt(context), 700); const generated = result.data; card = { ...baseCard, ...generated, discovery_questions: [], recommended_products: Array.isArray(generated.recommended_products) ? generated.recommended_products : baseCard.recommended_products, objection_handles: { ...baseCard.objection_handles, ...(generated.objection_handles && typeof generated.objection_handles === "object" ? generated.objection_handles : {}) } }; model = result.model; } catch { /* a safe fallback is intentional */ }
    const admin = createAdminSupabaseClient(); const { data, error } = await admin.from("lead_intelligence_cards").insert({ contact_id: context.contact.id, company_id: context.company?.id ?? null, ...card, recommended_products: card.recommended_products, discovery_questions: [], objection_handles: card.objection_handles, generated_by_model: model }).select().single(); if (error) throw new Error(error.message); return NextResponse.json({ card: data, fallback: model === "rule-based" }); }

...
export function ColdCallCard({ card, onGenerate, loading, fallback }: { card: CardData | null; onGenerate: () => void; loading: boolean; fallback: boolean }) {
  ...
  return <Card className="p-5">
    ...
    {card ? <div className="mt-5 space-y-5 text-sm"><Block label="Opening line" value={card.cold_call_opener} strong /><Block label="Company at a glance" value={card.company_summary} /><Block label="Why this contact / why now" value={card.why_this_company} /><Block label="Gnani pitch angle" value={card.best_gnani_angle} />{pitches ? <ElevatorPitches pitches={pitches} /> : <Block label="Company-specific elevator pitch" value={card.personalized_pitch} strong />}<ListBlock label="Relevant Gnani capabilities" value={card.recommended_products} /><Block label="Suggested first use case" value={card.demo_use_case} /><ObjectBlock label="If they push back" value={card.objection_handles} /><Block label="Send-email line" value={card.send_email_line} /><Block label="Meeting ask" value={card.meeting_ask} strong /><Block label="Evidence level" value={card.source_confidence} /></div> : <p className="mt-5 text-sm text-zinc-500">Load a call brief after finding a prospect. Once a current brief is saved, it returns from the cache immediately.</p>}
  </Card>;
}
